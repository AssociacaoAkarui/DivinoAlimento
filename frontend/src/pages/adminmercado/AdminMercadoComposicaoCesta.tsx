...
export default function AdminMercadoComposicaoCesta() {
  ...
  const ciclo = useMemo(
    () => ({
      ...
      valorMaximo:
        mercadoAPI?.valorMaximoCesta || cestasData?.[0]?.valormaximo || 80.0,
      tipo: "Cesta",
    }),
    [cicloAPI, mercadoAPI, composicoesData, cestasData],
  );
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  {!podePublicar && (
                    <TooltipContent>
                      <p>
                        Selecione pelo menos um produto com quantidade maior que
                        zero.
                      </p>
                    </TooltipContent>
                  )}
      ...
      <AlertDialog open={showConfirmModal} onOpenChange={setShowConfirmModal}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              O valor atual da cesta (R${" "}
              {valorAtual.toFixed(2).replace(".", ",")}) excede o valor máximo
              permitido (R$ {ciclo.valorMaximo.toFixed(2).replace(".", ",")}).
