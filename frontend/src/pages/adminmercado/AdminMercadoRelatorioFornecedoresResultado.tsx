...
export default function AdminMercadoRelatorioFornecedoresResultado() {
  ...
  const isDataLoading = ofertasQueries.some((query) => query.isLoading);

  const todasEntregas = useMemo(() => {
    const entregas: ReturnType<typeof transformarOfertasParaRelatorio> = [];
  ...
  const entregasFiltradas = useMemo(() => {
    ...
    const filtradas = filtrarEntregas(todasEntregas, filtros);
  ...
  const handleExportCSV = async () => {
    try {
      ...
      exportFornecedoresCSV(entregasFiltradas, ciclosDataForExport);
  ...
  const handleExportPDF = async () => {
    try {
      ...
      exportFornecedoresPDF(entregasFiltradas, ciclosDataForExport, resumo);
  ...
  return (
    <ResponsiveLayout
      leftHeaderContent={
        <button
          onClick={() =>
            navigate("/adminmercado/relatorio-fornecedores/ciclo", {
              replace: true,
            })
          }
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{resumo.totalQuantidade}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">
