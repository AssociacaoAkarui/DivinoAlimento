...
export default function AdminComposicaoVendaDireta() {
  ...
  const [sortOrder, setSortOrder] = useState<
    "mais-vendidos" | "menos-vendidos" | "nenhum"
  >("nenhum");
  ...
  const fornecedores = useMemo(() => {
    const uniqueFornecedores = Array.from(
      new Set(ofertas.map((o) => o.fornecedor)),
    );
  ...
  const executarConsolidacao = () => {
    ...
    setTimeout(() => {
      ...
      console.warn("Dados consolidados:", payload);
    ...
    >
      <div className="space-y-6">
        ...
        <Card className="bg-green-50 border-green-200">
          <CardHeader>
            <CardTitle className="text-xl text-[#006C35]">
              Resumo do Ciclo
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="flex flex-col items-center p-4 bg-white rounded-lg shadow-sm">
                ...
                <p className="text-3xl font-bold text-[#006C35]">
              ...
              <div className="flex flex-col items-center p-4 bg-white rounded-lg shadow-sm">
                ...
                <p className="text-3xl font-bold text-[#006C35]">
                  {ofertas.length} produtos
                </p>
              </div>
              <div className="flex flex-col items-center p-4 bg-white rounded-lg shadow-sm">
                <p className="text-sm text-muted-foreground mb-2">
                  Saldo Total Remanescente
                </p>
                <p className="text-3xl font-bold text-amber-600">
                  {totais.saldoTotal} unidades
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              <div className="p-4 bg-muted/50 rounded-lg">
                ...
                <p className="text-lg font-semibold">
                  {formatBRL(totais.valorTotalOfertado)}
                </p>
              </div>
              <div className="p-4 bg-muted/50 rounded-lg">
                <p className="text-xs text-muted-foreground mb-1">
              ...
              <div className="p-4 bg-muted/50 rounded-lg">
                ...
                <p className="text-lg font-semibold">
                  {totais.percentualVendido.toFixed(1)}%
                </p>
              </div>
              <div className="p-4 bg-muted/50 rounded-lg">
                <p className="text-xs text-muted-foreground mb-1">
          ...
          <CardHeader>
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Button
                  variant={
                    sortOrder === "mais-vendidos" ? "default" : "outline"
                  }
                  size="sm"
                  onClick={() =>
                    setSortOrder(
                      sortOrder === "mais-vendidos"
                        ? "nenhum"
                        : "mais-vendidos",
                    )
                  }
                ...
                <Button
                  variant={
                    sortOrder === "menos-vendidos" ? "default" : "outline"
                  }
                  size="sm"
                  onClick={() =>
                    setSortOrder(
                      sortOrder === "menos-vendidos"
                        ? "nenhum"
                        : "menos-vendidos",
                    )
                  }
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    /* TODO: filtrar apenas produtos com saldo */
                  }}
                >
                  ...
                  <TableBody>
                    {productGroups.flatMap((group) =>
                      group.variantes.map((variant) => {
                        const qtdVendida =
                          quantidadesVendidas.get(variant.id) || 0;
                        const valorFinal = variant.valor * qtdVendida;
                        const disponivel =
                          variant.quantidadeOfertada - qtdVendida;

                        return (
                          <TableRow key={variant.id}>
                            <TableCell className="font-medium">
                              {variant.produto_base}
                            </TableCell>
                            <TableCell>
                            ...
                            <TableCell className="text-right tabular-nums">
                              {formatBRL(variant.valor)}
                            </TableCell>
                            <TableCell className="text-center tabular-nums">
                              {variant.quantidadeOfertada}
                            </TableCell>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-3 pt-2">
              <p className="font-semibold text-amber-600">
                Ao confirmar, os dados serão consolidados e o mercado será
                encerrado.
              </p>
              <p>Essa ação não poderá ser desfeita.</p>
              <div className="bg-muted p-4 rounded-lg mt-4 space-y-2">
                <p className="text-sm">
                  <span className="font-semibold">Valor Total Vendido:</span>{" "}
                  {formatBRL(totais.valorTotalVendido)}
                </p>
                <p className="text-sm">
                  <span className="font-semibold">Unidades Vendidas:</span>{" "}
                  {totais.quantidadeTotalVendida}
                </p>
                <p className="text-sm">
                  <span className="font-semibold">Saldo Remanescente:</span>{" "}
                  {totais.saldoTotal} unidades
                </p>
              </div>
            </AlertDialogDescription>
