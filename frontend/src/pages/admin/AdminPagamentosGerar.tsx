...
const AdminPagamentosGerar = () => {
  ...
  const [editandoPagamento, setEditandoPagamento] = useState<any | null>(null);
  ...
  const { data: pagamentosData, refetch: refetchPagamentos } =
    useListarPagamentos();
  const gerarPagamentosMutation = useGerarPagamentosPorCiclo();
  ...
  const allCiclos = ciclosData?.listarCiclos || [];
  const ciclosFinalizados = Array.isArray(allCiclos)
    ? allCiclos.filter((c: any) => c.status === "finalizado")
    : [];
  const allPagamentos = pagamentosData?.listarPagamentos || [];
  const pagamentos = Array.isArray(allPagamentos) ? allPagamentos : [];
  ...
  const ciclosSelecionadosData = ciclosFinalizados.filter((c: any) =>
    ciclosSelecionados.includes(c.id),
  );
  const todosFinalizados = ciclosSelecionadosData.every(
    (c: any) => c.status === "finalizado",
  );

  const gerarListaPagamentos = async () => {
    if (ciclosSelecionados.length === 0) {
      toast({
        title: "Nenhum ciclo selecionado",
        description: "Selecione pelo menos um ciclo para gerar pagamentos.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      for (const cicloId of ciclosSelecionados) {
        await gerarPagamentosMutation.mutateAsync({
      ...
      toast({
        ...
        description: error.message || "Ocorreu um erro ao gerar os pagamentos.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleSalvarEdicao = async () => {
    if (editandoPagamento) {
      try {
        ...
        if (novoStatus === "pago" || novoStatus === "Pago") {
          ...
          await atualizarPagamentoMutation.mutateAsync({
            ...
            input: {
              ...
              status: novoStatus.toLowerCase().replace(" ", "_"),
              dataPagamento: novaDataPagamento || undefined,
              observacao: novaObservacao || undefined,
            },
          });
  ...
  const handleExcluirPagamento = async (id: string) => {
    try {
      await deletarPagamentoMutation.mutateAsync({ id });
      await refetchPagamentos();
      toast({
  ...
  return (
    ...
    >
      <div className="container max-w-6xl mx-auto py-8 px-4">
        <div className="mb-6">
          <RoleTitle
            page="Gerar Lista de Pagamentos"
            className="text-3xl mb-2"
          />
          <p className="text-muted-foreground">
            Gere automaticamente os registros de pagamentos de fornecedores e
            consumidores de ciclos finalizados.
          </p>
        </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <p className="font-semibold">{ciclosSelecionados.length}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">
                    Pagamentos Gerados
                  </p>
                  <p className="font-semibold">{pagamentos.length}</p>
                </div>
              </div>
                        ...
                        <td className="py-3 px-4">
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              size="icon-sm"
                              onClick={() =>
                                handleExcluirPagamento(pagamento.id)
                              }
