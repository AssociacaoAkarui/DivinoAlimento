...
export default function AdminOferta() {
  ...
  const [editingOferta, setEditingOferta] = useState<OfertaProduto | null>(
    null,
  );
  ...
  const [precoBaseSugerido, setPrecoBaseSugerido] = useState<number | null>(
    null,
  );
  const [salvandoOferta, setSalvandoOferta] = useState(false);
  const [certificacao, setCertificacao] = useState<CertificacaoType | "">("");
  const [tipoAgricultura, setTipoAgricultura] = useState<
    TipoAgriculturaType | ""
  >("");
  ...
  const getValorTotal = () => {
    return ofertas.reduce(
      (sum, o) => sum + o.valor_unitario * o.quantidade_disponivel,
      0,
    );
  };
          ...
          <CardContent className="p-0 space-y-6">
            ...
            <div className="flex flex-col items-center gap-4 md:flex-row md:items-start md:justify-between">
              <Badge
                ...
                className={
                  periodoOfertaAberto
                    ? "bg-green-600 hover:bg-green-700 text-white whitespace-nowrap md:order-2"
                    : "bg-orange-500 hover:bg-orange-600 text-white whitespace-nowrap md:order-2"
                }
              >
                {periodoOfertaAberto
                  ? "Período de oferta aberto"
                  : "Período de oferta encerrado"}
              </Badge>
            ...
            <div className="flex items-center justify-center gap-3 py-4">
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`flex items-center justify-center w-8 h-8 rounded-full ${ofertaSalva ? "bg-green-600" : "bg-green-600"} text-white font-bold`}
                >
              ...
              <div
                className={`h-0.5 w-12 md:w-20 ${ofertaSalva ? "bg-green-600" : "bg-muted"}`}
              />

              {/* Etapa 3 - Pendente ou Completa */}
              <div className="flex items-center gap-2">
                <div
                  className={`flex items-center justify-center w-8 h-8 rounded-full ${ofertaSalva ? "bg-green-600 text-white" : "border-2 border-muted text-muted-foreground"} font-bold`}
                >
                  3
                </div>
                <span
                  className={`text-sm font-medium ${ofertaSalva ? "text-foreground" : "text-muted-foreground"}`}
                >
                  Oferta enviada
                </span>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  value={valorUnitario}
                  onChange={(e) =>
                    setValorUnitario(formatBRLInput(e.target.value))
                  }
                    ...
                    <CardContent className="pt-6">
                      <fieldset>
                        ...
                        <RadioGroup
                          value={certificacao}
                          onValueChange={(value) =>
                            setCertificacao(value as CertificacaoType)
                          }
                        >
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center space-x-2">
                              <RadioGroupItem
                                value="convencional"
                                id="cert-convencional"
                              />
                              <Label
                                htmlFor="cert-convencional"
                                className="font-normal cursor-pointer"
                              >
                  ...
                  <Card>
                    <CardContent className="pt-6">
                      <fieldset>
                        ...
                        <RadioGroup
                          value={tipoAgricultura}
                          onValueChange={(value) =>
                            setTipoAgricultura(value as TipoAgriculturaType)
                          }
                        >
                          <div className="space-y-2">
                            <div className="flex items-center space-x-2">
                            ...
                            <div className="flex items-center space-x-2">
                              <RadioGroupItem
                                value="nao_familiar"
                                id="agri-nao-familiar"
                              />
                              <Label
                                htmlFor="agri-nao-familiar"
                                className="font-normal cursor-pointer"
                              >
                        ...
                        {!tipoAgricultura && selectedProdutoId && (
                          <p className="text-xs text-destructive mt-2">
                            Selecione o tipo de agricultura
                          </p>
                        )}
                      </fieldset>
                    </CardContent>
          ...
          <CardContent>
            {ofertas.length === 0 ? (
              <p className="text-center text-muted-foreground py-8">
                Nenhum produto adicionado. Use o formulário acima para adicionar
                produtos à oferta.
              </p>
            ) : (
              <>
                ...
                <div className="hidden md:block overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {ofertas.map((oferta) => {
                        const total =
                          oferta.valor_unitario * oferta.quantidade_disponivel;
                        const precoAlterado =
                          oferta.valor_unitario !== oferta.preco_base;

                        return (
                          <TableRow key={oferta.id}>
                            <TableCell className="font-medium">
                              {oferta.produto_base_nome}
                            </TableCell>
                            <TableCell>{oferta.unidade}</TableCell>
                            <TableCell>
                              {oferta.peso
                                ? `${oferta.peso.toFixed(2)} kg`
                                : oferta.volume
                                  ? `${oferta.volume.toFixed(2)} L`
                                  : "-"}
                            </TableCell>
                            <TableCell>
                              {formatBRL(oferta.preco_base)}
                            </TableCell>
                            <TableCell>
                              {precoAlterado ? (
                                <Badge
                                  variant="outline"
                                  className="bg-amber-50 dark:bg-amber-950 border-amber-300 dark:border-amber-700"
                                >
                      ...
                      <Card key={oferta.id} className="p-4 overflow-hidden">
                        <div className="space-y-3">
                          ...
                          <div className="flex justify-between items-start gap-3">
                            <h3 className="font-semibold text-base break-words flex-1 min-w-0">
                              {oferta.produto_base_nome}
                            </h3>
                          ...
                          <div className="flex gap-2 text-sm text-muted-foreground flex-wrap items-center">
                            ...
                            <span className="break-words">
                              {precoAlterado ? (
                                <Badge
                                  variant="outline"
                                  className="bg-amber-50 dark:bg-amber-950 border-amber-300 dark:border-amber-700 text-xs"
                                >
                                  {formatBRL(oferta.valor_unitario)}
                                </Badge>
                              ) : (
                            ...
                            <span className="whitespace-nowrap">
                              Qtd: {oferta.quantidade_disponivel}
                            </span>
                          </div>
        ...
        <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Tem certeza que deseja remover este produto da oferta? Esta ação
