...
export const filterUsuariosByStatus = (
  usuarios: Usuario[],
  statusList: string[],
): Usuario[] => {
...
export const filterUsuariosByPerfis = (
  usuarios: Usuario[],
  perfisList: string[],
): Usuario[] => {
...
export const normalizeUsuario = (usuario: UsuarioRaw): Usuario => {
  return {
    id: usuario.id,
    nome: usuario.nome,
    ...
    perfis: usuario.perfis.map(
      (p: string) => p.charAt(0).toUpperCase() + p.slice(1),
    ),
  };
};

export const normalizeUsuarios = (usuarios: UsuarioRaw[]): Usuario[] => {
