...
const BannerCarousel: React.FC<BannerCarouselProps> = ({ className }) => {
  ...
  const getBadgeText = (banner: CycleBanner): string => {
    switch (banner.badgeType) {
      case "current":
        return banner.cycleType === "semanal"
          ? "Semanal (7 dias)"
          : "Quinzenal (15 dias)";
      case "offers":
        return "3 dias";
  ...
  const getCountdownTarget = (banner: CycleBanner): Date => {
    if (banner.offerEnd && banner.badgeType === "offers")
      return banner.offerEnd;
    if (banner.extrasEnd && banner.badgeType === "extras")
      return banner.extrasEnd;
