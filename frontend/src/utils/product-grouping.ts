...
export function groupAndSortProducts(ofertas: Oferta[]): ProductGroup[] {
  ...
  const groups: ProductGroup[] = Object.entries(grouped).map(
    ([produto_base, variantes]) => {
    ...
    .map((group) => {
      ...
      if (filtros?.unidades?.length) {
        variantes = variantes.filter((v) =>
          filtros.unidades!.includes(v.unidade),
        );
      }
      if (filtros?.fornecedores?.length) {
        variantes = variantes.filter((v) =>
          filtros.fornecedores!.includes(v.fornecedor),
        );
      }
      ...
      if (filtros?.disponivelMin !== undefined) {
        variantes = variantes.filter(
          (v) => v.quantidadeOfertada >= filtros.disponivelMin!,
        );
      }

      // Novos filtros de certificação e tipo de agricultura
      if (
        filtros?.certificacoes &&
        filtros.certificacoes.size > 0 &&
        filtros.certificacoes.size < 3
      ) {
        variantes = variantes.filter(
          (v) => v.certificacao && filtros.certificacoes!.has(v.certificacao),
        );
      }
      if (
        filtros?.tiposAgricultura &&
        filtros.tiposAgricultura.size > 0 &&
        filtros.tiposAgricultura.size < 2
      ) {
        variantes = variantes.filter(
          (v) =>
            v.tipo_agricultura &&
            filtros.tiposAgricultura!.has(v.tipo_agricultura),
        );
      }
      ...
      return {
        ...
        minPreco:
          variantes.length > 0
            ? Math.min(...variantes.map((v) => v.valor))
            : group.minPreco,
        totalVariantes: variantes.length,
      };
    })
